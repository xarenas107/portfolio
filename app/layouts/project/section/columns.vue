...
section(class="flex flex-col gap-8")
  ...
  div(class='grid grid-rows-1 grid-cols-1 sm:grid-rows-2 sm:grid-cols-2 lg:grid-rows-3 lg:grid-cols-3 gap-16 pt-8 sm:pt-12 md:pt-20 lg:pt-24 xl:pt-32 z-20 lg:m-auto')
    nuxt-layout(v-for='{ title, description, src }, index in items' name='project-section-paragraph' class='lg:col-start-(--place) row-start-(--place)' :style='{ "--place": index + 1 }' :title :description :src small-title)
</template>
