...
div
	u-container(class="min-h-svh py-24 pt-4 sm:pt-6 lg:pt-24 flex flex-col gap-16 relative")
		div(class='flex flex-col gap-8 z-20')
			u-image(v-if='cover' :src='cover' :alt='title' :img-attrs='{ class: "w-full h-full" }' class="w-full sm:aspect-2/1 md:aspect-3/1 object-cover rounded-lg ring bg-elevated h-64 parallel-animation" loading='lazy' format='webp' width='1200' height='400')
